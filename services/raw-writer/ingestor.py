...
def check_schema(conn) -> None:
    ...
    views = [
        ("public.v_mqtt_publishers_config",
         ["device_id", "mqtt_topic", "broker_host", "broker_port", "broker_username",
          "broker_password", "broker_transport", "broker_ws_path", "broker_id", "broker_name"]),
        ("public.v_data_point_mappings_with_topic",
         ["device_asset_id", "json_key", "data_point_id", "logical_asset_id",
          "tag", "scale", "offset", "data_type"]),
...
def load_broker_configs() -> List[BrokerConfig]:
    ...
    for (device_id, device_name, broker_id, broker_name, mqtt_topic,
         broker_host, broker_port, broker_username, broker_password,
         broker_transport, broker_ws_path) in rows:

        if not broker_host or not mqtt_topic:
            logger.warning("Skipping device %s (%s): missing broker_host or mqtt_topic",
        ...
        if transport not in ("tcp", "websockets"):
            logger.warning("Device %s (%s): unknown broker_transport '%s', falling back to 'tcp'",
                           device_name, device_id, broker_transport)
            transport = "tcp"
        ...
        if "+" in topic or "#" in topic:
            logger.warning("Configured MQTT topic contains wildcard: %s (device %s). "
                           "This is fine for subscription but will NOT match exact topic->device mapping for republish/data map. "
                           "Ensure publisher topic is concrete in v_mqtt_publishers_config.", topic, dev_id)
    ...
    for cfg in configs.values():
        logger.info("Broker %s:%s (%s, transport=%s, ws_path=%s) topics: %s",
            cfg.broker_host, cfg.broker_port, cfg.broker_name or cfg.broker_id or "?",
            cfg.broker_transport, cfg.broker_ws_path or "", ", ".join(sorted(cfg.topics)))
...
class MQTTWorker(threading.Thread):
    def __init__(self, cfg: BrokerConfig):
        ...
        if transport == "websockets":
            ...
            logger.info("Using WebSockets for %s:%s with path %s",
                        self.cfg.broker_host, self.cfg.broker_port, ws_path)
        ...
        self.client.will_set(f"{self.topic_root}/status", json.dumps({"state": "offline"}), qos=0, retain=True)
    ...
    def run(self) -> None:
        logger.info("Starting worker for broker %s:%s (%s, transport=%s, client_id=%s)",
            self.cfg.broker_host, self.cfg.broker_port,
            self.cfg.broker_name or self.cfg.broker_id or "?",
            self.cfg.broker_transport, self.client._client_id.decode())
        ...
        while True:
            try:
                logger.info("Connecting MQTT client to %s:%s ...",
                            self.cfg.broker_host, self.cfg.broker_port)
                self.client.connect(self.cfg.broker_host, self.cfg.broker_port, keepalive=60)
                self.client.loop_forever()
            except Exception as e:
                logger.error("MQTT connection error for %s:%s: %s",
                             self.cfg.broker_host, self.cfg.broker_port, e, exc_info=True)
                time.sleep(5)

    # ----- MQTT callbacks ---------------------------------------------------

    def on_connect(self, client: mqtt.Client, userdata, flags, rc):
        logger.info("Connected rc=%s to %s:%s; subscribing to %d topics",
                    rc, self.cfg.broker_host, self.cfg.broker_port, len(self.cfg.topics))
        if rc == 0:
            # Heartbeat online
            try:
                client.publish(f"{self.topic_root}/status",
            ...
            logger.error("Failed to connect to MQTT broker %s:%s (rc=%s)",
                         self.cfg.broker_host, self.cfg.broker_port, rc)
    ...
    def _maybe_flush_metrics(self):
        ...
        if now - self._last_metrics_flush >= METRICS_INTERVAL:
            ...
            if METRICS_MQTT:
                try:
                    self.client.publish(f"{self.topic_root}/metrics",
        ...
        if now - self._last_heartbeat >= HEARTBEAT_INTERVAL:
            ...
            try:
                self.client.publish(f"{self.topic_root}/heartbeat",
    ...
    def on_message(self, client: mqtt.Client, userdata, msg: mqtt.MQTTMessage):
        ...
        if not device_id:
            ...
            logger.debug("Message on %s has no device mapping (exact match). "
                         "If config uses wildcards, exact topic->device map will miss. "
                         "Store the concrete topic in v_mqtt_publishers_config.", topic)
        ...
        for key, raw_val in data.items():
            ...
            try:
                if mp.data_type in ("BOOL", "BOOLEAN"):
                    base_val = 1.0 if (raw_val if isinstance(raw_val, bool) else float(raw_val) != 0.0) else 0.0
                else:
            ...
            per_key_publish_payloads.append((mp.logical_asset_id, mp.data_point_id, mp.tag, scaled))
    ...
    def on_disconnect(self, client: mqtt.Client, userdata, rc):
        if rc != 0:
            logger.warning("Unexpected MQTT disconnect from %s:%s (rc=%s)",
                           self.cfg.broker_host, self.cfg.broker_port, rc)
        try:
            client.publish(f"{self.topic_root}/status", json.dumps({"state": "offline"}), qos=0, retain=True)
        except Exception:
            pass
...
def main() -> None:
    ...
    logger.info("Flags: DISABLE_VALUES=%s DISABLE_REPUBLISH=%s METRICS_MQTT=%s DEBUG_SAMPLE_N=%s",
                DISABLE_VALUES, DISABLE_REPUBLISH, METRICS_MQTT, DEBUG_SAMPLE_N)

    configs = load_broker_configs()
    if not configs:
        logger.warning("No broker configs found; staying idle. Check v_mqtt_publishers_config.")

    workers: List[MQTTWorker] = []
    for cfg in configs:
        if not cfg.topics:
            logger.warning("Broker %s:%s has no topics; skipping worker",
